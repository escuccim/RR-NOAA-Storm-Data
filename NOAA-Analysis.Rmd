...
Now we will look at the top 5 types of events by mean damage and by total damage. To do this I sort the economic damage summary data by both total and mean, take the top 5 events for each, and then take the subset of the data which corresponds to the set of max events for either mean or total. This leaves 8 unique events.

```{r econsummary}
es_top_total <- econ_dmg_summary[order(desc(econ_dmg_summary$TOTAL))[1:5],]$EVTYPE
es_top_mean <- econ_dmg_summary[order(desc(econ_dmg_summary$MEAN))[1:5],]$EVTYPE
